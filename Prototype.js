...
Bullet.prototype.move = function() {
	switch (this.direct) {
		case 0:
			this.nextY =this.y - this.speed;
			if (!this.isBlock) {this.y = this.nextY;}
			break;
		case 1:
			this.nextX =this.x + this.speed;
			if (!this.isBlock) {this.x = this.nextX;}
			break;
		case 2:
			this.nextY =this.y + this.speed;
			if (!this.isBlock) {this.y = this.nextY;}
	...
	for (var m = 0; m < allTank.length; m++) {
		...
		if (!this.isBlock) {
			if (this.x > temp.x - 19 && this.x < temp.x + 19 && this.y > temp.y - 19 && this.y < temp.y + 19) {
		...
		for (var j = 0; j < allTank[i].bullets.length; j++) {
			for (var m = i + 1; m < allTank.length; m++) {
				for (var n = 0; n < allTank[m].bullets.length; n++) {
					coincident(allTank[i].bullets[j], allTank[m].bullets[n], 8,8);
				}
			}
